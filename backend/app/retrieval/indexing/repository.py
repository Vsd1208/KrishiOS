...
class RetrievalIndexRepository:
    ...
    async def create(self, index: RetrievalIndexVersion) -> RetrievalIndexVersion:
    ...
    async def get(self, index_id: int) -> RetrievalIndexVersion | None:
        """Load an index version by primary key."""
        return await self.session.get(RetrievalIndexVersion, index_id)

    async def list(self) -> Sequence[RetrievalIndexVersion]:
        """List index versions newest first."""
        result = await self.session.scalars(
            select(RetrievalIndexVersion).order_by(desc(RetrievalIndexVersion.created_at))
        )
        return result.all()

    async def active_for_alias(self, alias_name: str) -> RetrievalIndexVersion | None:
    ...
    async def previous_active_for_alias(self, alias_name: str) -> RetrievalIndexVersion | None:
    ...
    async def states_for_index(self, index_id: int) -> Sequence[IndexedDocumentState]:
        """Return document indexing states for an index version."""
        result = await self.session.scalars(
            select(IndexedDocumentState).where(IndexedDocumentState.index_version_id == index_id)
        )
        return result.all()
...
class RetrievalDocumentRepository:
    """Read access to Sprint 2 documents and chunks for index builds."""
    ...
    async def load_indexable_chunks(
        ...
        statement = (
            ...
            .order_by(KnowledgeDocument.id, DocumentChunk.chunk_index)
        )
