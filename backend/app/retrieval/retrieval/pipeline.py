...
from app.retrieval.interfaces.providers import EmbeddingProvider, RerankerProvider, VectorStoreProvider
...
class EnterpriseRetrievalPipeline:
    ...
    async def search(
        ...
        raw_hits = await self._retrieve(query_vector, merged_filters, top_k, score_threshold, include_delta)
        fresh_hits = [hit for hit in raw_hits if self._is_fresh(hit, merged_filters.effective_at)]
        deduped_hits = self._deduplicate(fresh_hits)
        reranked_hits = await self._reranker.rerank(query, deduped_hits)
        ranked = self._rank(reranked_hits, merged_filters, top_k)
        latency_ms = (perf_counter() - started) * 1000
        return RetrievalPipelineResult(query=query, latency_ms=latency_ms, results=ranked)
    ...
    def _rank(
        ...
        for hit in hits:
            score, signals = self._ranking_engine.score(hit, filters)
            scored.append(
        ...
        return sorted(scored, key=lambda result: result.ranking_score, reverse=True)[:top_k]
    ...
    @staticmethod
    def _is_fresh(hit: RetrievalHit, effective_at: datetime | None) -> bool:
